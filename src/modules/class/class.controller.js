...
import {
    fromYYYYMMDDToDDMMYYYY as formatDateReadable
} from '../../utils/date.js';
...
const getYearFromSemesterNumber = (semesterNumber) => {
    ...
    if (semesterNumber === 3 || semesterNumber === 4) return 'SE';
    else
    if (semesterNumber === 5 || semesterNumber === 6) return 'TE';
    else
    if (semesterNumber === 7 || semesterNumber === 8) return 'BE';
                    ...
                    include: [{
                        ...
                        where: {
                            [Op.and]: [
                                ...
                                ...(academicStartYearOfSemester ? [{
                                    academicStartYear: academicStartYearOfSemester
                                }] : []),
                                ...(academicEndYearOfSemester ? [{
                                    academicEndYear: academicEndYearOfSemester
                                }] : [])
                            ]
            ...
            [Op.and]: [{
                ...
                },
                {
                    [Op.or]: [{
                        ...
                        {
                            startTime: {
                                [Op.lte]: classObj.startTime
                            },
                            endTime: {
                                [Op.gte]: classObj.endTime
                            }
                        }
                    ]
    ...
    const cancelledClasses = await CancelledClass.findAndCountAll({
        ...
        include: [{
            ...
            where: {
                [Op.and]: [
                    ...(divisionId ? [{
                        timetableId: timetableOfDivision.id
                    }] : []),
                    ...(batchId ? [{
                        batchId: batchId
                    }] : [])
...
const bulkCreateClasses = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < classes.length; i++) {
            ...
            const teacher = await Teacher.findByPk(classData.teacherId, {
                transaction
            });
            if (!teacher) {
                errors.push({
            ...
            const course = await Course.findByPk(classData.courseId, {
                transaction
            });
            if (!course) {
                errors.push({
            ...
            const room = await Room.findByPk(classData.roomId, {
                transaction
            });
            if (!room) {
                errors.push({
            ...
            const timetable = await Timetable.findByPk(classData.timetableId, {
                transaction
            });
            if (!timetable) {
                errors.push({
            ...
            if (classData.batchId) {
                const batch = await Batch.findByPk(classData.batchId, {
                    transaction
                });
                if (!batch) {
                    errors.push({
            ...
            const overlapWindow = {
                [Op.or]: [{
                        startTime: {
                            [Op.between]: [classData.startTime, classData.endTime]
                        }
                    },
                    {
                        endTime: {
                    ...
                    {
                        startTime: {
                            [Op.lte]: classData.startTime
                        },
                        endTime: {
                            [Op.gte]: classData.endTime
                        }
                    }
                ]
            ...
            if (roomConflict || teacherConflict) {
                errors.push({
                    index: i,
                    error: 'scheduling conflict: overlapping time slot',
                    type: 'conflict'
                });
                continue;
            }
...
const bulkDeleteClasses = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const {
            Attendance
        } = await import('../../db/models/attendance.model.js');
        ...
        try {
            ...
            for (let i = 0; i < classes.length; i++) {
                ...
                try {
                    ...
                    let batch = null;
                    if (classData.batchId) {
                        batch = await Batch.findByPk(classData.batchId, {
                            transaction
                        });
                        if (!batch) {
                            errors.push({

...
describe('Teacher API - PUT /api/v1/teachers', () => {
    ...
    test('should return 401 when no token is provided', async () => {
        const res = await request(app).put(`/api/v1/teachers/${teacher.id}`).send({
            id: teacher.id,
            firstName: 'New'
        });
    ...
    test('should allow teacher to update details (role allowed)', async () => {
        ...
        const res = await request(app).put(`/api/v1/teachers/${teacher.id}`).set('Authorization', `Bearer ${teacherToken}`).send({
            id: teacher.id,
            firstName: 'NewName'
        });

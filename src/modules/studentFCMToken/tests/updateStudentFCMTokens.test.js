...
describe('Student FCM Token API - PUT /api/v1/student-fcm-tokens', () => {
    ...
    const payload = (studentId, fcmToken) => ({
        studentId,
        fcmToken
    });
    ...
    describe('Validation errors', () => {
        test('should return 400 when studentId is missing', async () => {
            const res = await request(app).put('/api/v1/student-fcm-tokens').set('Authorization', `Bearer ${adminToken}`).send({
    ...
    test('should update token successfully', async () => {
        ...
        const updated = await StudentFCMToken.findOne({
            where: {
                studentId: student.id,
                fcmToken: 'fcm_token_12345'
            }

...
const getSemesters = {
    query: Joi.object().keys({
        ...
        isEven: Joi.boolean().default(true).messages({
            'boolean.base': 'isEven must be a boolean'
        }),
        isOdd: Joi.boolean().default(true).messages({
            'boolean.base': 'isOdd must be a boolean'
        })
    })
};
...
const addSemester = {
    body: Joi.object().keys({
        ...
        semesterNumber: Joi.number().integer().min(1).max(20).required().messages({
            'any.required': 'Semester number is required'
        }),
        academicStartYear: Joi.number().integer().min(1900).max(3000).required().messages({
            'any.required': 'Academic start year is required'
        }),
        academicEndYear: Joi.number().integer().min(1900).max(3000).required().messages({
            'any.required': 'Academic end year is required'
        }),
...
const bulkDeleteSemesters = {
    body: Joi.object().keys({
        semesterIds: Joi.array().items(
            uuid.required().messages({
                'string.guid': 'Each semester ID must be a valid UUID'
            })
        ).min(1).max(50).required().
        messages({
